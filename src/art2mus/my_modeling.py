...
@dataclass
class MyAudioLDM2ProjectionModelOutput(BaseOutput):
    """
    Args:
...
class MyAudioLDM2ProjectionModel(ModelMixin, ConfigMixin):
    """
    ...
    def __init__(self, text_encoder_dim, text_encoder_1_dim, langauge_model_dim):
        ...
        """ ImageBind projection layer. Input features: 1024. """
        self.projection_2 = nn.Linear(1024, langauge_model_dim)
    ...
    def forward(
        ...
        print("USING MY MODELING FILE")
        if not (hidden_states_1 is None and attention_mask_1 is None):
        
            hidden_states = self.projection(hidden_states) # hidden_states shape: [1,1,512] || attention_mask: [1,1]
            hidden_states, attention_mask = add_special_tokens( # hidden_states shape: [1,1,768] || attention_mask here: [1,1]
                hidden_states, attention_mask, sos_token=self.sos_embed, eos_token=self.eos_embed
            )
            # hidden_states shape: [1,3,768] || attention_mask here: [1,3]
            hidden_states_1 = self.projection_1(hidden_states_1) # hidden_states_1 shape: [1,15,1024] || attention_mask_1: [1,15]
            hidden_states_1, attention_mask_1 = add_special_tokens( # hidden_states_1 shape: [1,15,768] || attention_mask_1: [1,15]
                hidden_states_1, attention_mask_1, sos_token=self.sos_embed_1, eos_token=self.eos_embed_1
            )
            # hidden_states_1 shape: [1,17,768] || attention_mask_1: [1,17]
            # concatenate clap and t5 text encoding
            hidden_states = torch.cat([hidden_states, hidden_states_1], dim=1)
            # hidden_states shape: [1,20,768]
            ...
            # attention_mask shape: [1,20,768] || attention_mask_1: [1,17]
            ...
            hidden_states = self.projection_2(hidden_states) # hidden_states shape: X || attention_mask: X
            hidden_states, attention_mask = add_special_tokens( # hidden_states shape: X || attention_mask here: X
                hidden_states, attention_mask, sos_token=self.sos_embed_2, eos_token=self.eos_embed_2
            )
            if attention_mask is None:
                attention_mask = torch.ones(hidden_states[:2])           
            
        return MyAudioLDM2ProjectionModelOutput(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
        )


class MyAudioLDM2UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
